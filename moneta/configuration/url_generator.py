...
class UrlGenerator(BaseModel):
    ...
    def _signature_chunks(self, order_id: str, amount: float) -> str:
        signature_chunks = [
    ...
    def signature(self, order_id: Union[str,int], amount: float) -> Optional[str]:
        if self.config.basic_config.account_code is None:
            return None
        sign = self._signature_chunks(str(order_id), amount)
    ...
    def payment_url(
        ...
        query = QueryPayload(
            ...
            signature=self.signature(order_id, amount),
        )
